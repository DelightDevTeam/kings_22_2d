 ...
 <div class="col-12">
  <div class="card">
   ...
   <div class="card-header pb-0">
    <div class="d-lg-flex">
     <div>
      <h5 class="mb-0">3D ပွဲစဉ်များ Dashboards</h5>
   ...
   <div class="table-responsive">
    {{-- <h1>Match Times for {{ $currentYear }}</h1> --}}

        {{-- <h2 class="text-center">Current Match Times</h2>
        @if($currentMatchTimes->isNotEmpty())
            <ul>
                @foreach($currentMatchTimes as $matchTime)
                    <li>{{ $matchTime->result_date }}: {{ $matchTime->match_time }}</li>
                @endforeach
            </ul>
        @else
            <p>No match times for today.</p>
        @endif --}}

        <h2 class="text-center">All Match Times for the Year {{ $currentYear }}</h2>
      ...
      <thead class="thead-light">
            ...
            <tbody>
               @foreach($yearMatchTimes as $index => $matchTime)
               <tr>
                <td>{{ $index++}}</td>
                <td>{{ $matchTime->result_date }} </td>
                <td> {{ $matchTime->match_time }} </td>
               </tr>
                @endforeach
            </tbody>
        </table>
